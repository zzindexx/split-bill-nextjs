...
import { stringToState } from './helpers/shareHelper';

const App: React.FC<{ data?: string }> = ({ data }) => {
  let currentState: SplitBillState;

  if (data) {
    currentState = stringToState(data);
  } else {
  ...
  if (state.participants.length > 0 && state.payments.length > 0) {
    ...
    pageTitle = `Split the ${formattedValue(total)} bill for ${state.participants.length} people`;
  }
